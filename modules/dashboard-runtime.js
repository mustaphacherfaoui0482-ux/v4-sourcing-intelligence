...
function buildCanonical(opportunity, economics, decision = null) {
  return buildRadarOpportunity({
    id: opportunity.id,
...
export function calculateDashboardState(opportunity = DEMO_OPPORTUNITY) {
  ...
  // First build the canonical opportunity so the Decision Engine consumes
  // the authoritative evidence confidence rather than a stale declared value.
  const canonicalWithoutDecision = buildCanonical(opportunity, economics);
  const decision = evaluateOpportunity({
    ...opportunity,
    confidence: canonicalWithoutDecision.confidence.score,
    potentialScore: canonicalWithoutDecision.score,
    landedCost: opportunity.offer.landedCost,
    margin: economics.netContributionMargin,
  });
  const canonical = buildCanonical(opportunity, economics, decision);
...
function wireActions(state) {
  ...
  exportButton.addEventListener('click', () => {
    ...
    anchor.href = url; anchor.download = 'v4-sourcing-intelligence-report.json'; anchor.click(); URL.revokeObjectURL(url);
  }, { once: true });
}
export function initDashboardRuntime(opportunity = DEMO_OPPORTUNITY) {
  const state = calculateDashboardState(opportunity); const { viewModel } = state;
  updateKpis(viewModel); updatePhone(viewModel); updateCostAdvice(state.economics); updateSignals(state.opportunity); wireActions(state);
  document.documentElement.dataset.v4Runtime = 'ready'; window.V4SourcingRuntime = Object.freeze(state); return state;
}
