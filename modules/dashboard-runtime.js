...
import { getActiveOpportunity } from './opportunity-runtime-bridge.js';
...
function updateKpis(viewModel, economics) { const cards = [...document.querySelectorAll('.kpis .kpi')]; if (cards.length < 5) return; setGauge(cards[0].querySelector('.gauge'), Number(viewModel.score)); setText(cards[0].querySelector('.good'), Number(viewModel.score) >= 75 ? 'Excellent potentiel' : Number(viewModel.score) >= 50 ? 'Potentiel à approfondir' : 'Potentiel faible'); setText(cards[1].querySelector('.val'), money(economics.inputs.landedCost)); setText(cards[2].querySelector('.val'), viewModel.dimensions.margin); setText(cards[3].querySelector('.val'), money(viewModel.economics.maxCacAtTargetMargin)); setText(cards[4].querySelector('.decision'), viewModel.decision); setDecisionStyle(cards[4].querySelector('.decision'), viewModel.decision); setText(cards[4].querySelector('.sub'), viewModel.decisionReason); }
function updatePhone(viewModel, economics) { const phone = document.querySelector('.phone'); if (!phone) return; setGauge(phone.querySelector('.phoneg'), Number(viewModel.score)); const cards = [...phone.querySelectorAll('.pc')]; if (cards.length < 4) return; setText(cards[0].querySelector('b'), money(economics.inputs.landedCost)); setText(cards[1].querySelector('b'), viewModel.dimensions.margin); setText(cards[2].querySelector('b'), money(viewModel.economics.maxCacAtTargetMargin)); setText(cards[3].querySelector('b'), viewModel.decision); setDecisionStyle(cards[3].querySelector('b'), viewModel.decision); }
...
function updateSignals(viewModel) { const signals = [...document.querySelectorAll('.signals .sig')]; if (signals.length < 5) return; const d = viewModel.dimensions ?? {}; const m = viewModel.marketSignals ?? {}; setText(signals[0].querySelector('span'), `Demande : ${Number(d.demand) || 0}/100`); setText(signals[1].querySelector('span'), `Concurrence : ${m.competition}`); setText(signals[2].querySelector('span'), `Saturation marché : ${m.saturation}`); setText(signals[3].querySelector('span'), `Risque fournisseur : ${Number(d.risk) || 0}/100`); setText(signals[4].querySelector('span'), `Délai de production : ${m.productionLeadTimeDays}`); }
function updateSuppliers(suppliers) { const table = [...document.querySelectorAll('.table')].find((node) => /Comparaison des fournisseurs/i.test(node.closest('.panel')?.textContent ?? '')); if (!table || !Array.isArray(suppliers)) return; const tbody = table.querySelector('tbody'); if (!tbody) return; tbody.textContent = ''; suppliers.forEach((supplier, index) => { const row = document.createElement('tr'); const landed = (Number(supplier.unitPrice) || 0) + (Number(supplier.shippingCost) || 0); const quality = Number(supplier.score) >= 90 ? '★★★★★' : Number(supplier.score) >= 80 ? '★★★★☆' : '★★★☆☆'; const cells = [String(index + 1), supplier.name ?? '—', supplier.country === 'CN' ? '🇨🇳' : (supplier.country ?? '—'), money(landed), `${Number(supplier.moq) || 0} pcs`, `${Number(supplier.leadTimeDays) || 0} j`, quality, Math.round(Number(supplier.score) || 0)]; cells.forEach((value, cellIndex) => { const cell = document.createElement('td'); if (cellIndex === 1) { const name = document.createElement('b'); name.textContent = value; cell.appendChild(name); } else if (cellIndex === 6) { cell.className = 'stars'; cell.textContent = value; } else if (cellIndex === 7) { const score = document.createElement('span'); score.className = 'score'; score.textContent = value; cell.appendChild(score); } else cell.textContent = value; row.appendChild(cell); }); row.title = `${supplier.composition ?? ''} · ${supplier.customization ? 'Personnalisation' : 'Sans personnalisation'} · ${supplier.verificationStatus === 'verified' ? 'Vérifié' : supplier.verificationStatus === 'rejected' ? 'Rejeté' : 'À vérifier'}`; tbody.appendChild(row); }); }
...
export function initDashboardRuntime(opportunity = getActiveOpportunity() ?? DEMO_OPPORTUNITY) { const state = calculateDashboardState(opportunity); const { viewModel } = state; updateKpis(viewModel, state.economics); updatePhone(viewModel, state.economics); updateCostAdvice(state.economics); updateCostBreakdown(state.opportunity.costBreakdown, state.opportunity.dimensions.landedCost); updateRadar(state.opportunity); updateSignals(viewModel); updateSuppliers(state.opportunity.suppliers); wireActions(state); document.documentElement.dataset.v4Runtime = 'ready'; window.V4SourcingRuntime = Object.freeze(state); return state; }
export function refreshDashboardRuntime() { return initDashboardRuntime(getActiveOpportunity() ?? DEMO_OPPORTUNITY); }
if (typeof document !== 'undefined') { if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => refreshDashboardRuntime(), { once: true }); else refreshDashboardRuntime(); }
