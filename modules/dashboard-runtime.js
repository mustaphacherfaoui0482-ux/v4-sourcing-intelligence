...
function render(state){
  ...
  if(signals.length>=5){
    ...
    setText(signals[2].querySelector('span'),empty?'—':vm.dimensions.margin);
...
function sourcingStatus(product,stage,done=false){document.querySelector('[data-v4-sourcing-run]')?.remove();const el=document.createElement('div');el.dataset.v4SourcingRun='true';Object.assign(el.style,{position:'fixed',left:'50%',top:'18px',transform:'translateX(-50%)',zIndex:'10000',width:'min(520px,calc(100vw - 24px))',padding:'12px 14px',border:`1px solid ${done?'#315c3d':'#5b4a27'}`,borderRadius:'10px',background:'#0d1012',color:'#f3f4f2',font:'600 11px Inter,system-ui,sans-serif',boxShadow:'0 14px 40px #0009'});
  const head=document.createElement('div');Object.assign(head.style,{display:'flex',justifyContent:'space-between'});const title=document.createElement('span');title.textContent='SOURCING';const status=document.createElement('b');status.textContent=done?'TERMINÉ':'EN COURS';status.style.color=done?'#79df31':'#d7b45a';head.append(title,status);
  const productLine=document.createElement('div');productLine.textContent=product;productLine.style.marginTop='6px';productLine.style.color='#9da4a8';
  const stageLine=document.createElement('div');stageLine.textContent=`Étape : ${stage}`;stageLine.style.marginTop='7px';stageLine.style.color=done?'#79df31':'#d7b45a';
  el.append(head,productLine,stageLine);document.body.appendChild(el);if(done)window.setTimeout(()=>el.remove(),2600)}

function startSourcing(product){const next=buildManualOpportunity(product);if(!next)return null;sourcingStatus(next.product,'Fiche P0 créée');saveActiveOpportunity(next);initDashboardRuntime(next);feedback(`Fiche P0 créée : ${next.product}`);return next}
function askProduct(){const value=window.prompt('Produit à analyser');return value?.trim()||null}
