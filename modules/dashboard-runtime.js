...
function updateSignals(viewModel) {
  const signals = [...document.querySelectorAll('.signals .sig')];
  if (signals.length < 5) return;
  const d = viewModel.dimensions ?? {};
  const m = viewModel.marketSignals ?? {};
  setText(signals[0].querySelector('span'), `Demande : ${Number(d.demand) || 0}/100`);
  setText(signals[1].querySelector('span'), `Concurrence : ${m.competition}`);
  setText(signals[2].querySelector('span'), `Saturation marché : ${m.saturation}`);
  setText(signals[3].querySelector('span'), `Risque fournisseur : ${Number(d.risk) || 0}/100`);
  setText(signals[4].querySelector('span'), `Délai de production : ${m.productionLeadTimeDays}`);
}
