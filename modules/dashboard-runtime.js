...
function engineInput(o) {
  return { demand: o.demandScore, marketing: o.marketingScore ?? o.demandScore, sourcing: o.sourcingScore,
    profitability: o.profitabilityScore, confidence: o.confidence, economics: calculateOfferEconomics(o.offer) };
}
...
export function calculateDashboardState(o = DEMO_OPPORTUNITY) {
  ...
  const canonical = buildRadarOpportunity({ id: o.id, product: o.product, source: o.source, country: o.country,
    radarSignals: engineInput(o), dimensions: { potential: o.potential, demand: o.demandScore, margin: economics.netContributionMargin,
      availability: o.availability, landedCost: o.landedCostScore, risk: o.riskScore, easeOfTest: o.easeOfTest, dataConfidence: o.confidence },
    economics, decision });
...
function render(state) {
  ...
  if (cards.length >= 5) {
    ...
    setText(cards[4].querySelector('.decision'), vm.decision);
...
function feedback(text) {
  document.querySelector('[data-v4-feedback]')?.remove();
  const el = document.createElement('div'); el.dataset.v4Feedback = 'true'; el.textContent = text;
  Object.assign(el.style, { position:'fixed', right:'18px', bottom:'18px', zIndex:'9999', padding:'10px 13px', border:'1px solid #5b4a27', borderRadius:'8px', background:'#11100d', color:'#d7b45a', font:'600 11px Inter,system-ui,sans-serif', boxShadow:'0 10px 30px #0008' });
  document.body.appendChild(el); window.setTimeout(() => el.remove(), 1800);
}

function sourcingStatus(product, stage, done = false) {
  document.querySelector('[data-v4-sourcing-run]')?.remove();
  const el = document.createElement('div'); el.dataset.v4SourcingRun = 'true';
  Object.assign(el.style, { position:'fixed', left:'50%', top:'18px', transform:'translateX(-50%)', zIndex:'10000', width:'min(520px,calc(100vw - 24px))', padding:'12px 14px', border:`1px solid ${done ? '#315c3d' : '#5b4a27'}`, borderRadius:'10px', background:'#0d1012', color:'#f3f4f2', font:'600 11px Inter,system-ui,sans-serif', boxShadow:'0 14px 40px #0009' });
  el.innerHTML = `<div style="display:flex;justify-content:space-between"><span>FENNEC · SOURCING</span><b style="color:${done ? '#79df31' : '#d7b45a'}">${done ? 'TERMINÉ' : 'EN COURS'}</b></div><div style="margin-top:6px;color:#9da4a8">${product}</div><div style="margin-top:7px;color:${done ? '#79df31' : '#d7b45a'}">Étape : ${stage}</div>`;
  document.body.appendChild(el); if (done) window.setTimeout(() => el.remove(), 2600);
}

function startSourcing(product = DEMO_OPPORTUNITY.product) {
  const name = String(product || '').trim(); if (!name) return null;
  const stages = ['Découverte','Analyse','Scoring','Rentabilité','Validation','Décision']; let i = 0;
  sourcingStatus(name, stages[i]);
  const next = { ...DEMO_OPPORTUNITY, id:`manual-${Date.now()}`, product:name };
  initDashboardRuntime(next);
  const timer = window.setInterval(() => { i += 1;
    if (i >= stages.length) { window.clearInterval(timer); sourcingStatus(name, 'Décision disponible', true); feedback(`Sourcing terminé : ${name}`); return; }
    sourcingStatus(name, stages[i]);
  }, 450);
  return next;
}
...
export function initDashboardRuntime(opportunity = DEMO_OPPORTUNITY) {
  try { const state=calculateDashboardState(opportunity); render(state); wireActions(state); document.documentElement.dataset.v4Runtime='ready'; window.V4SourcingRuntime=Object.freeze(state); return state; }
  catch(error) { document.documentElement.dataset.v4Runtime='error'; console.error('[V4 Sourcing] runtime initialization failed',error); sourcingStatus(opportunity?.product || 'Produit inconnu','Erreur d’initialisation',true); return null; }
}
