...
export default async function handler(req, res) {
  ...
  let fetched = null; let directError = null; let providerError = null; let readerDiagnostics = null; let readerUsed = false; let productCandidates = [];
  ...
  if (!searchUrl && (!fetched || extractionStatus(fetched.extracted) !== 'COMPLETE')) {
    try {
  ...
  const evidenceStatus = searchUrl ? (productCandidates.length ? 'CANDIDATES_ONLY' : 'INSUFFICIENT') : (status === 'EMPTY' ? 'INSUFFICIENT' : status === 'COMPLETE' ? 'EXTRACTED' : 'PARTIAL');
  return send(res, 200, { ok: true, source: 'Alibaba.com', sourceUrl: url, acquisition: fetched.acquisition || (readerUsed ? 'DIRECT+JINA_READER' : 'DIRECT'), acquisitionUrl: fetched.acquisitionUrl || url, searchUrl, productCandidates, extracted: searchUrl ? { product: null, displayedPrice: null, moq: null, supplier: null, supplierCountry: null } : fetched.extracted, fetchStatus: 'page_retrieved', extractionStatus: status, acquisitionStatus, evidenceStatus, directError, providerError, providerConfigured, readerDiagnostics: readerDiagnostics || diagnostics(fetched) });
}
