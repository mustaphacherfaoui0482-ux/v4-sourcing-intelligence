...
export function extractionStatus(extracted = {}) { const present = EXTRACTION_KEYS.filter((key) => extracted[key] !== null && extracted[key] !== undefined && String(extracted[key]).trim() !== '').length; if (present === 0) return 'EMPTY'; if (present === EXTRACTION_KEYS.length) return 'COMPLETE'; return 'PARTIAL'; }
export function mergeAlibabaExtraction(primary = {}, secondary = {}) { const merged = {}; for (const key of EXTRACTION_KEYS) { const primaryValue = primary[key]; const secondaryValue = secondary[key]; const primaryPresent = primaryValue !== null && primaryValue !== undefined && String(primaryValue).trim() !== ''; const secondaryPresent = secondaryValue !== null && secondaryValue !== undefined && String(secondaryValue).trim() !== ''; merged[key] = primaryPresent ? primaryValue : secondaryPresent ? secondaryValue : null; } return Object.freeze({ ...merged, parserStatus: extractionStatus(merged) === 'EMPTY' ? 'NO_STRUCTURED_DATA' : 'PARTIAL_OR_COMPLETE' }); }
...
function extractCanonicalAlibabaUrl(html, baseUrl) {
  ...
  const patterns = [
    ...
    /(https?:\/\/[^"'<>\s]+\.alibaba\.com\/product-detail\/[^"'<>\s]+)/i,
    /(\/product-detail\/[^"'<>\s]+?\.html(?:\?[^"'<>\s]+)?)/i,
  ];
  for (const pattern of patterns) { const match = source.match(pattern); if (match?.[1]) candidates.push(match[1]); }
  for (const candidate of candidates) { try { const absolute = new URL(candidate, baseUrl).href; const normalized = normalizeAlibabaUrl(absolute); if (normalized && isAlibabaHostname(new URL(normalized).hostname)) return normalized; } catch {} }
  return null;
}
...
async function fetchAlibabaPage(url) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const response = await fetch(current, { redirect: 'manual', signal: AbortSignal.timeout(TIMEOUT_MS), headers: { 'user-agent': 'Mozilla/5.0 (compatible; V4-Sourcing-Intelligence/1.0; +https://vercel.com)', accept: 'text/html,application/xhtml+xml' } });
    if (response.status >= 300 && response.status < 400) { const location = response.headers.get('location'); const next = normalizeAlibabaUrl(location ? new URL(location, current).href : ''); if (!next || !isAlibabaHostname(new URL(next).hostname)) throw new Error('alibaba_redirect_blocked'); current = next; continue; }
    if (!response.ok) throw new Error(`alibaba_http_${response.status}`);
    const contentType = response.headers.get('content-type') || ''; if (!contentType.includes('text/html')) throw new Error('alibaba_non_html_response');
    const contentLength = Number(response.headers.get('content-length') || 0); if (contentLength > MAX_BYTES) throw new Error('alibaba_response_too_large');
    const text = await response.text(); if (Buffer.byteLength(text, 'utf8') > MAX_BYTES) throw new Error('alibaba_response_too_large');
    return { url: current, html: text, acquisition: 'DIRECT', contentType, canonicalUrl: extractCanonicalAlibabaUrl(text, current) };
  }
...
export default async function handler(req, res) {
  ...
  if (!fetched || extractionStatus(fetched.extracted) !== 'COMPLETE') {
    try {
      ...
      else { fetched = { ...fetched, acquisition: after !== before ? 'DIRECT+JINA_READER' : fetched.acquisition, acquisitionUrl: readerFetched.acquisitionUrl || fetched.acquisitionUrl, extracted: merged }; readerUsed = after !== before; }
    } catch (error) {
