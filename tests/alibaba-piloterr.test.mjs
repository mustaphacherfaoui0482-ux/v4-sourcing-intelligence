...
test('Piloterr documented top-level Alibaba response normalizes title, price, MOQ and supplier', () => {
  ...
  assert.equal(result.providerCanonicalUrl, 'https://www.alibaba.com/product-detail/Custom-Product_1601175379813.html');
});

test('Piloterr nested response remains supported for compatibility', () => {
  const result = normalizePiloterrProduct({
    product: {
      product_id: 1601175379813,
      title: 'Custom Product',
